...
const drawUserAnswers = async (ctx, answers, isLoading) => {
	...
	answers.forEach((answer, i) => {
			...
			ctx.fillText(150 * (11 - i), cv(ax + 160), cv(ay + 50));

			ctx.fillStyle = '#5F5F5F';
			ctx.font = `${cv(26)}px SetoFont-SP`;
			ctx.fillText(username, cv(ax + 120), cv(ay + 92));
			console.log("Draw user anem:", username);
		})

}
const drawQuestionTFEnd = async (canvas, bg, data, genImg = false) => {
	...
	if (imageImg != null && imageImg != undefined) {
        ctx.drawImage(imageImg, 740, 270, 450, 250)
    }
    ctx.textAlign = 'left';
    ctx.textBaseline = 'middle';
	...
	if (isLoading) {
		...
		ctx.fillStyle = '#707070';
		ctx.font = `${cv(40)}px SetoFont-SP`;
		ctx.fillText('Retrieve user answer...' , cv(960), cv(900));
	}
	else {
		await drawUserAnswers(ctx, userAnswers)
	}
	if (genImg) {
		const name = `/generated/question_tf_end_screen_${time}.jpeg`
